...
<?php
  function individualMoviePeopleFactPageQuery($movieSLUGPeople, $job) {
    ...
    // 2. Perform database query
    $query = $newconnection->prepare("SELECT FirstName, MiddleInitialName, LastName FROM movies_peoples INNER JOIN peoples_jobs ON peoples_jobs.PeopleID = movies_peoples.people_id INNER JOIN jobs ON peoples_jobs.JobID = jobs.JobID INNER JOIN peoples ON movies_peoples.people_id = peoples.PeopleID INNER JOIN movies ON movies_peoples.movie_id = movies.movie_id WHERE MoviePageLink = ? and jobs = ? ");
