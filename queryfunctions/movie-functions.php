...
<?php
  function individualMovieFactPageQuery($movieTitle, $city) {
    ...
    // 2. Perform database query
    $query = $newconnection->prepare("SELECT * FROM movies_cities INNER JOIN movies ON movies.movie_id = movies_cities.movie_id INNER JOIN cities ON  = movies_cities.city_id WHERE MovieTitle = ? AND city = ? ");
...
<?php
  function individualMovieFactPageLocationQuery($movieSLUG) {
    ...
    // 2. Perform database query
    $query = $newconnection->prepare("SELECT * FROM movies_cities INNER JOIN movies ON movies.movie_id = movies_cities.movie_id INNER JOIN cities ON  = movies_cities.city_id WHERE MoviePageLink = ? ");
