...
<?php
  function individualMoviePeopleFactPageQuery($movieSLUGPeople, $job) {
    ...
    // 2. Perform database query
    $query = $newconnection->prepare("SELECT FirstName, MiddleInitialName, LastName FROM movies_peoples INNER JOIN peoples_jobs ON peoples_jobs.PeopleID = movies_peoples.PeopleID INNER JOIN jobs ON peoples_jobs.JobID = jobs.JobID INNER JOIN peoples ON movies_peoples.PeopleID = peoples.PeopleID INNER JOIN movies ON movies_peoples.movie_id = movies.MovieID WHERE MoviePageLink = ? and jobs = ? ");
