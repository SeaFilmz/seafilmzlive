...
<?php
  function cityFilmedMovieTableQuery($city, $StateProvince) {
    ...
    // 2. Perform database query
    $query = $newConnection->prepare("SELECT * FROM movies_cities INNER JOIN movies ON movies.movie_id = movies_cities.movie_id INNER JOIN cities ON cities.city_id = movies_cities.city_id WHERE city = ?  AND state_province = ? ORDER BY movie_title ASC ");
...
<?php
  function cityAttractionQuery($city, $attractionType) {
    ...
    // 2. Perform database query
    $query = $newConnection->prepare("SELECT * FROM attractions_cities INNER JOIN attractions ON attractions.attraction_id = attractions_cities.attraction_id INNER JOIN cities ON cities.city_id = attractions_cities.city_id WHERE city = ? AND attraction_type = ? ORDER BY attraction_name ASC ");
