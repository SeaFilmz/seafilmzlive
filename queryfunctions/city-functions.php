<?php
  function cityQuery($cityLinkSlug) {
    ...
    $query = $newconnection->prepare("SELECT * FROM `cities_counties` INNER JOIN cities ON cities.CityID = cities_counties.CityID INNER JOIN counties ON counties.CountyID = cities_counties.CountyID WHERE CityLinks = ? ");
