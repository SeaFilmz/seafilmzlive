<?php
  function cityQuery($cityLinkSlug) {
    ...
    $query = $newconnection->prepare("SELECT * FROM `cities_counties` INNER JOIN cities ON cities.CityID = cities_counties.CityID INNER JOIN counties ON counties.county_id = cities_counties.CountyID WHERE CityLinks = ? ");
