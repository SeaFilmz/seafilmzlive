      ...
      <table class="PeoplesCountTable">
        <tr>
          ...
          <?php
              ...
              $query = $newconnection->prepare("SELECT COUNT(*) peoplecount FROM peoples_jobs INNER JOIN peoples ON peoples.PeopleID = peoples_jobs.PeopleID INNER JOIN jobs ON jobs.JobID = peoples_jobs.JobID INNER JOIN cities ON cities.CityID = peoples.BirthCityID WHERE City = ? AND jobs = ? AND MusicianName IS NOT NULL ");
